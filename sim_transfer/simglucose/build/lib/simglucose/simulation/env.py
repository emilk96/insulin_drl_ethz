...
class T1DSimEnv(object):
    ...
    def step(self, action, reward_fun=risk_diff):
        ...
        hrs48 = int(22 * 60 / self.sample_time) #22
        BG_last_hour = self.CGM_hist[-window_size:]
        reward = reward_fun(BG_last_hour)

        done = BG < 20 or BG > 400 or len(self.CGM_hist) > hrs48 #Changed to let each episode run for a maximum of 22 hrs

        if BG < 20:
            reward = reward - 1000000 #Hypoglycemic -1e6 termination penalty 
        elif len(self.CGM_hist) > hrs48:
            print("Nice, system ended an episode!!")
        else:
            pass
    ...
    def render(self, close=False):
        ...
        normal_bg, low_bg, high_bg = self.viewer.render(self.show_history())
