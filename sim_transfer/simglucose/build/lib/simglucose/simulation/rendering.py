...
class Viewer(object):
    ...
    def render(self, data):
        ...
        #New from here
        bg_levels = data['BG'].values
        length = len(data['BG'].values)
        low_bg = np.count_nonzero(bg_levels < 70)
        high_bg = np.count_nonzero(bg_levels > 180)
        normal_bg = length-low_bg-high_bg
        low_bg = low_bg/length
        high_bg = high_bg/length
        normal_bg = normal_bg/length
...
def adjust_xlim(ax, timemax, xlabel=False):
    ...
    if timemax > xlim[1] - timedelta(minutes=30):
        xmax = xlim[1] + timedelta(hours=20) #Changed from 6
        update = True
